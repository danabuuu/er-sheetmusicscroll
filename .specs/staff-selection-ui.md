...
- Each part has a short user-editable label chosen from a fixed set: **S, A, T, B** (Soprano, Alto, Tenor, Bass), presented in that order as a dropdown. No free-text entry. No row-count suffix — all strips are rendered as two rows tall. When "Build & Add Part" succeeds, the dropdown auto-advances to the next unused label in S→A→T→B order.
